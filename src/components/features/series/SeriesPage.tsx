            ...
            <header className="relative w-full h-[75vh] flex items-center overflow-hidden">
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-6 w-full pt-20 pb-5">
                    <div className="space-y-8">
                        <div className="flex flex-wrap gap-2">
                            {data.series.tags?.map((tag: string) => (
                                <Badge
                                    key={tag}
                                    variant="secondary"
                                    className="bg-white/10 hover:bg-white/20 backdrop-blur-md text-white border-white/20 px-4 py-1 uppercase tracking-tighter text-[10px]"
                                >
                                    {tag}
                                </Badge>
                            ))}
                        ...
                        <h1 className="text-6xl md:text-8xl font-black tracking-tighter leading-[0.85] uppercase text-white drop-shadow-sm">
                            {data.series.title}
                        </h1>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-end">
                            <p className="text-xl md:text-2xl font-light leading-relaxed max-w-xl text-zinc-200">
                                {data.series.desc}
                            </p>
                        ...
                        >
                            ...
                            <div className="md:col-span-6 flex flex-col justify-center space-y-6">
                                <div className="space-y-4">
                                    <h3 className="text-4xl md:text-5xl font-extrabold leading-[1.1] tracking-tighter transition-all group-hover:tracking-normal">
                                        {blog.title}
                                    </h3>
                                ...
                                <div className="flex items-center gap-4 pt-4">
                                    <div className="flex gap-2">
                                        {blog.tags.slice(0, 2).map((tag: string) => (
                                            <Badge key={tag} variant="outline" className="rounded-none font-black text-[9px] uppercase tracking-tighter">
                                                #{tag}
                                            </Badge>
                                        ))}
                                    </div>
