...
interface SeriesFormProps {
  ...
  trigger?: React.ReactNode; // Optional trigger for "New Series" mode
}
...
const SeriesForm = ({ id, open, setOpen, trigger }: SeriesFormProps) => {
  ...
  const { data: seriesData, isPending: isSeriesDataPending, isError } = useQuery({
    queryKey: ['user-series-by-id', id],
    queryFn: () => getSeriesFormData({ id: id! }),
    enabled: !!id,
  })
 ...
 const mutation = useMutation({
  ...
  mutationFn: ({ data, id }: { data: z.infer<typeof seriesFormSchema>; id?: string }) => {
    return id ? updateSeries(id, data) : createSeries(data);
  },
  onSuccess: () => {
    // Invalidate both the list and the specific item
    queryClient.invalidateQueries({ queryKey: ["series"] });
    if (id) {
      queryClient.invalidateQueries({ queryKey: ["user-series", id] });
    }
  ...
  onError: (error) => {
    console.error(error);
    toast.error(id ? "Failed to update series" : "Failed to create series");
  },
});
